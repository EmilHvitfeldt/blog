...
Here we will fit the models using both our training data, first using the stop words, then using the simple would count approach. This will take a while as we are dealing with over 1000 features in both cases.
...
Tidymodels includes the pyardstick](https://github.com/tidymodels/yardstick) wpackage hich makes evaluation calculations much easier and tidy. It can allow us to calculate the accuracy by calling the `accuracy()` function
...
WAnd we see that the stop words model eats the naive model (one that always picks the majority class), while lacking behind the word count model.
...
TFurthermore I hope to have showed the power of tidymodels. idymodels is still growing, so if you have any feedback/bug reports/suggests please go to the respective repositories, we would highly appreciate it!
