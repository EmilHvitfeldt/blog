...
data <- read_csv("https://github.com/EmilHvitfeldt/blog/raw/master/data/socialmedia-disaster-tweets-DFE.csv")
```
...
```{r data_dtm, message=FALSE, error=FALSE}
...
This leaves us with `r nrow(words_10)` features. We create this meta data.frame which acts as a intermediate from our first data set since some tweets might have disappeared completely after the reduction.
...
and we get an accuracy of `r svm_cm$overall[["Accuracy"]]`.
...
and we get an accuracy of `r nb_cm$overall[["Accuracy"]]`.
...
and we get an accuracy of `r logitboost_cm$overall[["Accuracy"]]`.
...
and we get an accuracy of `r rf_cm$overall[["Accuracy"]]`.
...
```{r nnet_mod, message=FALSE, warning=FALSE}
...
and we get an accuracy of `r nnet_cm$overall[["Accuracy"]]`.
