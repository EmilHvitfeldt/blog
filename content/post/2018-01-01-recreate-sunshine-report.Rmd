...
knitr::include_graphics("/img/2018-01-01-13.53scan.jpg")
```
...
```{r include=FALSE}
load(glue("~/Documents/Blog/data/{airport_code}-{year}.Rdata"))
```
...
```{r, fig.asp=1/2, out.width='100%', fig.align='center'}
full_data2 %>%
...
```{r echo=FALSE, fig.asp=1/2, out.width='100%', fig.align='center'}
load(glue("~/Documents/Blog/data/KNYC-{year}.Rdata"))
full_data2 %>%
  mutate(con = Conditions == "Clear",
         day_minutes2 = lag(day_minutes)) %>%
  filter(sun_up) %>%
