...
```{r include=FALSE}
img <- image("~/Documents/Academia/Github/blog/static/img/parrot.jpg")
```
...
```{r include=FALSE}
img <- image("~/Documents/Academia/Github/blog/static/img/balls.jpg")
plot(img)
```
...
For our further calculations we need to know what colorspace this image is in
...
```{r include=FALSE}
my_img <- image("~/Documents/Academia/Github/blog/static/img/file45425287b9e9.png")
```
...
Like before we found the blue balls by identifying the region in the BGR colorspace where its blue, we expand the same idea to the other colors. (I have not attempted brown as it is fairly similar in color to the table)
